...
- **EVAL script numkeys key [key ...] arg [arg ...]:**numkeys 是key的个数，后边接着写key1 key2... val1 val2....
...
- **SCRIPT LOAD script** ：把脚本加载到脚本缓存中，返回SHA1校验和。
...
- **EVALSHA sha1 numkeys key [key ...] arg [arg ...]** ：根据缓存执行对应的lua脚本
...
- **SCRIPT EXISTS script [script ...]** 、**SCRIPT FLUSH** 
...
**redis.call()命令和redis.pcall()命令的使用和区别**

```java
// 在lua脚本中，使用redis.call()或者redis.pcall()命令来调用redis的命令。
eval "redis.call('set',KEYS[1],ARGV[1])" 1 K1 'V1'

eval "return redis.call('get',KEYS[1])" 1 K1
// 两个命令的唯一区别在于对错误处理的不同，redis.call()在执行过程中如果发生错误，脚本会停止执行，并返回错误。
redis.pcall()执行出错的话脚本不会停止，最终会返回一个lua脚本的错误。
```

**二进制位数组**

- **二进制数组底层是一个向量数组，数组内只保存0或者1。redis底层使用SDS实现bitArray**

```java
setbit bitmap 100 1 ; 表示将二进制数组bitmap中第100位的值设置为1.

getbit bitmap 100 ; 表示获取二进制数组dbitmap中第100位的值

bitcount bitmap ; 表示获取二进制数组bitmap所有值为1的个数。

bitop and res x y;  逻辑与（&）操作二进制数组x，y，将结果存入res二进制数组中。
bitop or res x y;	或（|）操作二进制数组x，y，将结果存入res二进制数组中。
bitop xor res x y;	异或（^）操作二进制数组x，y，将结果存入res二进制数组中。
bitop not res x y;	逻辑非（~）操作二进制数组x，y，将结果存入res二进制数组中。
```
...
![image-20210711173825636](总结知识点.assets/image-20210711173825636.png)

![image-20210711174047910](总结知识点.assets/image-20210711174047910.png)



**物理结构**

- **Mysql通过文件系统对数据和索引进行存储。在物理结构上分为日志文件和索引文件。日志文件使用顺序io读写，数据文件使用随机IO读写。**

**日志文件**

**错误日志（errorlog）；记录mysql在运行中遇到的严重的错误信息，5.7之后默认打开。**

**二进制日志（binlog）：记录所有的ddl和dml语句，dml语句需要提交事务之后记录。以事件的形式保存，描述了数据的变更顺序。主要用于主从，备份等功能。**

**慢查询日志：记录慢sql，调优使用默认关闭；slow_query_log=ON 开启慢查询日志；long_query_time=3;sql执行时间超过3秒就记录为慢日志。**

**重做日志（redolog）：数据库崩溃回复时使用。**

**回滚日志（undo log）：事务回滚和MVCC时使用。**
...
**中继日志（relay log）：主从复制时使用。**

**InnoDB引擎架构**

![image-20210713173101949](总结知识点.assets/image-20210713173101949.png)

- **表空间：主要存储所有表数据，以及系统的表，列，索引、字段等。可分为用户表空间和系统表空间。**
- **段（segment）：段是表空间文件中的主要组织结构，它是一个逻辑概念，用来管理物理文件，是构成索引、表、回滚段的基本元素。**
- **区/簇（extents）：簇是由64个连续的页组成的，每个页大小为16KB，即每个簇的大小为1MB。簇是构成段的基本元素，一个段由若干个簇构成。一个簇是物理上连续分配的一个段空间，每一个段至少会有一个簇，在创建一个段时会创建一个默认的簇。如果存储数据时，一个簇已经不足以放下更多的数据，此时需要从这个段中分配一个新的簇来存放新的数据。一个段所管理的空间大小是无限的，可以一直扩展下去，但是扩展的最小单位就是簇。**
- **页（page）：逻辑上和物理上都是有序的。默认大小16K。向表中插入数据时，如果一个页面被写完，系统会在当前簇中分配一个新页来写入数据。**
- **Buffer Pool 缓冲池：其中包括数据页和索引页，插入缓存区，自适应哈希索引，锁信息以及数据字典信息（表结构，表字段，库名，表名，索引，视图等）。客户端每一次的sql语句的查询，会先查询内存是否命中，如果没有会查询磁盘中的记录，mysql每次会拿一页的数据，并放入Buffer Pool中。默认当前查询数据的周围更有概率会被再次查询到。**
- **Redo log buffer 重做日志缓冲区：执行DML语句时，首先通过数据引擎从磁盘中查询出对应的数据，首先会写入undo页，逻辑的记录数据修改前的数据状态，然后进行修改形成脏页，并且写入到redo log 缓冲区中。当事务提交时，先将sql语句记录到binlog中，在根据配置将redo log buffer中的数据写入到redo log 文件中，写入成功之后才算事务提交成功。undo page会等待purge线程回收。**
- **redo log buffer 的数据如何写入redo log的策略由参数innodb_flush_log_at_trx_commit来控制**

![image-20210714125435410](总结知识点.assets/image-20210714125435410.png)
...
**Double write：数据库在刷新脏数据的时候，首先会将脏页数据写入double write buffer中去，大小为2M，buffer空间满或其他条件触发，使double write buffer存的脏页先写到共享表空间，之后在写入数据文件（共享表空间是在ibdbata文件中划出2M连续的空间，专门给double write刷脏页用的，说白了就是磁盘上2M连续空间。）**

![image-20210714163455190](总结知识点.assets/image-20210714163455190.png)

**CheckPoint (检查点)：脏数据写入磁盘的时机。**

- **缩短数据库恢复的时间，buffer pool空间不够用的时候，将脏页刷新到磁盘，redolog不可用的时候，刷新脏页**
...
- 不可重复读：一个事务因读取到另一个事务已提交的update或者delete数据。导致对同一条记录
  读取两次以上的结果不一致。
- 幻读：一个事务因读取到另一个事务已提交的insert数据。导致对同一张表读取两次以上的结果不
  一致。 
...
- **快照读：普通的select操作就是快照读，即不加锁的非阻塞读。**
...
- **数据库中每条行记录都会包含3个隐藏字段；DB_TRX_ID：最近修改的事务id，DB_ROLL_PTR：回滚指针，指向该数据的上一个版本的记录，DB_ROW_ID：隐含的自增ID（隐藏主键），如果数据表没有主键，InnoDB会生成一个隐藏主键产生一个聚簇索引。**
- **Read View(读视图)：Read View就是事务进行快照读操作的时候生产的读视图(Read View)，在该事务执行的快照读的那一刻，会生成数据库系统当前的一个快照，记录并维护系统当前活跃事务的ID(当每个事务开启时，都会被分配一个ID, 这个ID是递增的，所以最新的事务，ID值越大)，up_limit_id表示生成读视图时最小的还未提交事务id，low_limit_id表示生成视图时系统尚未分配的下一个事务ID（简单理解为最大的事务ID +1）**

**MVCC的整体流程：开启一个事务A时，会同步生成一个Read View，读视图中维护一个事务id的列表，up_limit_id，low_limit_id，此时事务A进行一个select查询操作，查出当前记录后，通过该记录的DB_TRX_ID来判读该是否可读，如果该记录的DB_TRX_ID 小于low_limit_id的值，表示查询的数据是在事务A 开启之前提交的，可以读到。如果该记录的DB_TRX_ID 大于up_limit_id的值，表示该记录是在事务A开启之后才修改的数据，不可读。如果该记录的 low_limit_id < DB_TRX_ID < up_limit_id的值，表示该记录是在A事务开启之后修改的，需要判断该事务ID是否在读视图中的事务列表中，如果不存在，则可读。如果存在则表示在事务A开启是，该事务id还未提交，通过该记录的回滚指针去查符合要求的快照记录读出来。**

**RC,RR级别下的InnoDB快照读有什么不同？**

- **RR级别下，每次开启事务的时候会生成一个Read View，在整个事务期间共用这个Read View。所以可以实现在同一个事务中数据的可重复读。**
- **RC级别下，每次快照读的时候都会生成一个Read View，在整个事务多次查询会生成多个Read View，所以RC级别下会出现可重复读。**

### Mysql索引

**索引：帮助Mysql高效获取数据的数据结构。能够加快数据库的查询速度。**

- **优点：提高数据的检索速度，减少IO。索引列对数据进行排序，降低数据排序成本。**
- **缺点：索引占据磁盘，不仅要保存数据文件还要保存索引文件。降低数据更新效率。**

**索引分类**

- **单列索引：包含主键索引，唯一索引，普通索引**
- **组合索引、全文索引，空间索引，位图索引**

![image-20210712124230785](总结知识点.assets/image-20210712124230785.png)

**索引的原理分析**

- **MyISAM和InnoDB存储引擎，仅支持B+树索引，MEMORY存储引擎，支持HASH索引和B+树索引。**

**B树和B+树的区别**

- **B树在叶子节点和非叶子节点都存有数据，B+树只在叶子节点上存入数据。**
- **B+树的叶子节点有横向指针，数据是有序排列的。**

**B+树的优势**

- **B+树的中间结点不保存数据，所以磁盘页中可以容纳更多的节点。**
- **B树只要匹配到节点（无论中间节点或者叶子节点）就可以查到数据，B+树必须要查到叶子节点才行。查询速度更稳定**
- **范围查找时，B+树只需要遍历叶子节点，B树则需要中序遍历整个树**

**聚集索引和非聚集索引**

- **MyISAM的索引文件和数据文件是分开存储，所以只有InnoDB才有聚集索引。**
- **InnoDB的主键索引是聚集索引，聚集索引键值的逻辑顺序决定了表中相应行的物理顺序。一个表中只有一个聚集索引。**
- **聚集索引的根节点存储的键值对是主键id和一条数据记录，非聚集索引存储的值是主键id，通过普通索引查询数据时，会先查到对应的主键值，然后再通过该主键值查询聚集索引树来查询表数据。该操作称为回表。可以通过创建组合索引，让查询的列都在组合索引内，来避免回表操作。该操作称为索引覆盖。**
- ![image-20210712152012783](总结知识点.assets/image-20210712152012783.png)

**索引的使用场景**

![image-20210712152138416](总结知识点.assets/image-20210712152138416.png)

**索引执行计划**

![image-20210712160327893](总结知识点.assets/image-20210712160327893.png)

**id**

- 每个select语句都会自动分配一个唯一标识
- id相同：执行顺序由上向下，id不同：如果是子查询，id号会自增，**id越大，优先级越高**
- id为null，表示为一个结果集，不使用它来进行查询
...
**不使用索引下推的话，存储引擎通过索引检索到数据，然后返回到mysql的服务器中，服务器通过where的判断条件进行过滤数据**
...
使用索引下推优化时，mysql会将索引name和age都下推到存储引擎去筛选数据，在查出数据的同时去判断是否符合条件。值返回需要的数据，减少存储引擎返回给服务器的数据，并且减少回表次数。
```

**索引失效分析**

![image-20210712164341273](总结知识点.assets/image-20210712164341273.png)
...
- **表共享读锁：不同的线程可以同时读，获取锁的线程可以读、写，但是没获取锁的线程只能读，不能写。**
- **表共享写锁：获取锁的线程可以读和写，没有获取锁的线程读写都会阻塞。**
...
- 每一条DML，DDL语句都会申请MDL锁，MDL加锁过程是系统自动控制，无法直接干预，读读共享，读写互斥，写写互斥。

**行级锁**

- **记录锁：锁定索引中的一条记录**
- **间隙锁（gap locks）：锁定记录前，记录中，记录后的行；解决InnoDB引擎在可重复的级别下幻读的问题**
- **next-key锁：记录锁+间隙锁；可重复的隔离级别，数据库通过行级锁和间隙锁共同组成。**

![image-20210713160650145](总结知识点.assets/image-20210713160650145.png)

**行级锁分类**

- **共享读锁（S）：一个线程读数据，其它线程可以读该数据，但是不能改。**
- **排他写锁（X）：一个线程获取了数据的排它锁，其它线程不能对该数据进行读写操作。**

```mysql
SELECT * FROM TABLE --- 普通的select无锁
SELECT * FROM TABLE WHERE ... LOCK IN SHARE MODE --- 共享读锁
SELECT * FROM TABLE WHERE ... FOR UPDATE --- 排他写锁

InnoDB是通过给索引上的索引项加锁来实现的，只有通过索引来检索数据才能使用行级锁，否则会升级为表锁
```

![image-20210713124006623](总结知识点.assets/image-20210713124006623.png)

### MySql优化

- **通过选择合适的存储引擎，表字段类型，数据库建表的三大范式等。**
- **合理创建索引，SQL层面优化、利用缓存，分库分表（水平拆分，垂直拆分）**
- **架构层面，主从复制、读写分离、集群等。**

**数据库服务器层面的优化**

- **buffer pool默认为128M，可以扩大为内存的3/4左右。让内存中可以加载更多的数据。**
- **内存预热**
- **降低磁盘写入次数；调整redolog的大小，大一点写入的次数少。innodb_log_file_size 设置成 innodb_buffer_pool_size * 0.25  。通用日志，慢查询日志关闭，以及redo log的写入策略innodb_flush_log_at_trx_commit 0 1 2  **
- **提高磁盘的读写能力，使用SSD**
